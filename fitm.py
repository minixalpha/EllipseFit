...
def getR1(da, db, emax1, emax2):
    r = ((db * db / da + (da * da - db * db) * emax1 / (da * da))
        + (db / (da * da)
        * ((da * da - db * db) * (c2d(2) * da - emax1) * emax1).sqrt()))
    x = da - r
    y = decimal.Decimal(0)
...
def getR2(da, db, emax1, emax2):
    r = ((da * da / db + (da * da - db * db) * emax2 / (db * db))
        - da / (db * db)
        * ((da * da - db * db) * (c2d(2) * db + emax2) * emax2).sqrt())
    x = decimal.Decimal(0)
    y = db - r
...
def drawEllipse(draw, da, db):
    bbox = (W / 2.0 - da / 2.0, H / 2.0 - db / 2.0,
            W / 2.0 + da / 2.0, H / 2.0 + db / 2.0)
    draw.ellipse(bbox, fill=128)


def drawCircle(draw, c):
    (r, x, y, _) = c
    (r, x, y) = (d2c(r), d2c(x), d2c(y))
    bbox = (x - r + W / 2.0, -(y + r) + H / 2.0,
            x + r + W / 2.0, -(y - r) + H / 2.0)
    draw.ellipse(bbox)


def drawAxis(draw, w, h):
    draw.line(((0, h / 2), (w, h / 2)), fill="rgb(0,255,0)")
    draw.line(((w / 2, 0), (w / 2, h)), fill="rgb(0,255,0)")


def draw(da, db, error, result):
    c1 = result[0]
    c2 = result[1]
    c3 = result[2]
    ...
    drawEllipse(draw, da, db)
    drawCircle(draw, c1)
    drawCircle(draw, c2)
    drawCircle(draw, c3)
    drawAxis(draw, W, H)
    img.show()
    fn = str(da) + "-" + str(db) + "-" + str(error)[:6] + ".jpg"
    img.save(fn)
...
def main():
    while True:
        ...
        draw(float(major_axis), float(minor_axis), float(emax), result)
        print ("\n")
